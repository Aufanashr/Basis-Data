...
CREATE DEFINER=`root`@`localhost` PROCEDURE `tambah_transaksi` (IN `p_id_pelanggan` INT, IN `p_id_buku` INT, IN `p_jumlah` INT)   BEGIN
    ...
    SELECT harga, stok INTO v_harga_buku, v_stok_saat_ini
    ...
    ELSE
        ...
        UPDATE pelanggan SET total_belanja = total_belanja + v_total_harga
...
(1, 'Pemrograman SQL', 'Andi Wijaya', 150000.00, 8),
(2, 'Database Sistem', 'Rina Marlina', 200000.00, 4),
(3, 'Bisnis Digital', 'Dedi Kurnia', 250000.00, 5);
...
(1, 'Budi Santoso', 300000.00, 'REGULER');
...
(1, 1, 1, 2, 300000.00, '2026-06-21'),
(2, 1, 2, 4, 800000.00, '2026-06-21');
